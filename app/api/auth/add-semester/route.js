// app/api/auth/add-semester/route.js
// Replaces: POST /api/auth/addsemester in backend/routes/userauth.js
...
export async function POST(request) {
  try {
    ...
    if (!semesterName || !subjects || subjects.length === 0) {
      return NextResponse.json({ success: false, error: "Semester name and subjects are required" }, { status: 400 });
    }

    // Grade to grade-point mapping (BUITEMS 4.0 scale)
    ...
    const processedSubjects = subjects.map((sub) => {
      ...
      return { name: sub.name, creditHours: Number(sub.creditHours), grade: sub.grade };
    });

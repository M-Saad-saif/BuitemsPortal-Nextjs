"use client";
// app/cgpa-calculator/page.js — Converted from src/components/CGPAcal.js
// Multi-semester CGPA calculator

import { useState, useCallback } from "react";

const emptySemester = (n) => ({ name: `Semester ${n}`, gpa: "", credits: "15" });

export default function CGPACalculatorPage() {
  const [semesters, setSemesters] = useState([emptySemester(1), emptySemester(2)]);
  ...
  const calculate = useCallback(() => {
    ...
    for (const s of semesters) {
      ...
      if (!isNaN(gpa) && !isNaN(cr) && cr > 0 && gpa >= 0 && gpa <= 4) {
        ...
        totalQP += qp;
        totalCredits += cr;
        rows.push({ name: s.name, gpa: gpa.toFixed(2), credits: cr, qp: qp.toFixed(2) });
      }
    }

    const cgpa = totalCredits > 0 ? (totalQP / totalCredits).toFixed(2) : "0.00";
  ...
  const getLabel = (val) => {
    const v = parseFloat(val);
    if (v >= 3.7) return "President's Honor Roll 🏆";
    if (v >= 3.3) return "Dean's Honor Roll 🌟";
    if (v >= 3.0) return "Good Standing ✅";
    if (v >= 2.0) return "Satisfactory";
    ...
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      {result && (
        <div className="card fade-in">
          <div className="text-center mb-6">
            <p className="text-sm text-gray-500 uppercase tracking-widest mb-1">Your CGPA</p>
            <p className={`text-6xl font-bold ${getColor(result.cgpa)}`}>{result.cgpa}</p>
            <p className="text-lg font-medium text-gray-600 mt-2">{getLabel(result.cgpa)}</p>
            <p className="text-sm text-gray-400 mt-1">
              Total Credits: {result.totalCredits} &nbsp;|&nbsp; Total Quality Points: {result.totalQP}
            </p>
          </div>
            ...
            <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  ...
                  background: "linear-gradient(to right, #1a3c6e, #2563eb)",
                }}
              />
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 text-xs uppercase text-gray-500">
                  ...
                  <th className="text-center px-3 py-2 rounded-r">Quality Points</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {result.rows.map((r, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className={`px-3 py-2 text-center font-bold ${getColor(r.gpa)}`}>{r.gpa}</td>
                    <td className="px-3 py-2 text-center text-gray-600">{r.credits}</td>
                    <td className="px-3 py-2 text-center text-gray-600">{r.qp}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-blue-50 font-semibold">
                  ...
                  <td className={`px-3 py-2 text-center font-bold text-xl ${getColor(result.cgpa)}`}>
                    {result.cgpa}
                  </td>
                  <td className="px-3 py-2 text-center text-gray-700">{result.totalCredits}</td>
                  <td className="px-3 py-2 text-center text-gray-700">{result.totalQP}</td>
                </tr>
              </tfoot>
            </table>
          </div>
      ...
      <div className="mt-6 bg-blue-50 border border-blue-100 rounded-xl p-4">
        <p className="text-sm text-blue-700">
          ...
          <a href="/portal" className="underline font-medium">Student Portal</a> to
          automatically track your CGPA across semesters.
        </p>
      </div>
    </div>
  );

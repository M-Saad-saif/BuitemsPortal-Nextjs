  ...
  const calculate = useCallback(() => {
    ...
    for (const sub of subjects) {
      ...
      if (!isNaN(credits) && credits > 0) {
        ...
        details.push({ name: sub.name || "Subject", credits, grade: sub.grade, gp, points: gp * credits });
      }
    }

    const gpa = totalCredits > 0 ? (totalPoints / totalCredits).toFixed(2) : "0.00";
    setResult({ gpa, totalCredits, totalPoints: totalPoints.toFixed(2), details });
  }, [subjects]);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 mt-2">Calculate your semester GPA using the 4.0 grading scale</p>
      </div>
      ...
      <div className="card mb-6">
        ...
        {/* Header row */}
        <div className="grid grid-cols-12 gap-2 mb-2 text-xs font-medium text-gray-500 uppercase tracking-wide px-1">
            ...
            <div key={i} className="grid grid-cols-12 gap-2 items-center">
              ...
              <div className="col-span-3">
                <select
                  ...
                  onChange={(e) => updateSubject(i, "creditHours", e.target.value)}
      ...
      {result && (
        <div className="card fade-in">
          <div className="text-center mb-6">
            <p className="text-sm text-gray-500 uppercase tracking-widest mb-1">Your Semester GPA</p>
            <p className={`text-6xl font-bold ${getGPAColor(result.gpa)}`}>{result.gpa}</p>
            <p className="text-lg font-medium text-gray-600 mt-2">{getGPALabel(result.gpa)}</p>
            <p className="text-sm text-gray-400 mt-1">
              Total Credits: {result.totalCredits} &nbsp;|&nbsp; Quality Points: {result.totalPoints}
            </p>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 text-gray-500 text-xs uppercase">
                  ...
                  <th className="text-center px-3 py-2 rounded-r">Quality Pts</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {result.details.map((d, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-center text-gray-600">{d.credits}</td>
                    <td className="px-3 py-2 text-center">
                      <span className={GRADE_COLORS[d.grade] || "text-gray-600"}>{d.grade}</span>
                    </td>
                    <td className="px-3 py-2 text-center text-gray-600">{d.gp.toFixed(1)}</td>
                    <td className="px-3 py-2 text-center font-medium text-gray-700">{d.points.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-blue-50 font-semibold">
                  ...
                  <td className="px-3 py-2 text-center text-gray-700">{result.totalCredits}</td>
                  <td colSpan={2}></td>
                  <td className="px-3 py-2 text-center text-gray-700">{result.totalPoints}</td>
                </tr>
              </tfoot>
            </table>
          </div>
      ...
      <div className="card mt-6">
        <h3 className="font-semibold text-gray-700 mb-3">BUITEMS Grading Scale</h3>
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 text-sm">
          {Object.entries(GRADE_POINTS).map(([g, p]) => (
            <div key={g} className="flex justify-between items-center bg-gray-50 px-3 py-1.5 rounded-lg">
              <span className={`font-bold ${GRADE_COLORS[g] || "text-gray-600"}`}>{g}</span>
